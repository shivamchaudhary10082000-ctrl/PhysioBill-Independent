...
function LoginPage({ onLogin }: { onLogin: (user: AuthUser) => void }) {
  return <div className="grid min-h-screen place-items-center bg-background p-4"><div className="w-full max-w-lg rounded-[28px] border bg-card p-8 shadow-sm"><div className="flex items-center gap-3"><PhysioBillBrand markClassName="h-12 w-12" wordmarkClassName="[&>span:first-child]:text-2xl" /><div><p className="text-sm text-muted-foreground">Phase-1 demo session</p></div></div><div className="mt-7 rounded-xl bg-secondary/60 p-4 text-sm text-muted-foreground">This is not production authentication. No password is stored. Real auth will replace this adapter in Phase 2.</div><div className="mt-6 grid gap-3 sm:grid-cols-2"><Button onClick={() => onLogin(demoAuthUser)}><LogIn size={16} /> Enter as physio</Button><Button variant="soft" onClick={() => onLogin(demoPatientAuthUser)}><UserRound size={16} /> Enter patient portal</Button></div></div></div>;
}
