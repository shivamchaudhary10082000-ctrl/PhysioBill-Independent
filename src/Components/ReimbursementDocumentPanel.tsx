...
export function ReimbursementDocumentPanel({ invoiceId }: { invoiceId: string }) {
  ...
  return (
    <section className="rounded-2xl border bg-card p-5 shadow-sm sm:p-6" aria-busy={busy}>
      ...
      <div className="sr-only" role="status" aria-live="polite">
        {busy ? 'Issuing reimbursement verification record.' : copied ? 'Verification link copied.' : ''}
      </div>
      ...
      {document && verificationUrl && (
        <>
          <div className="no-print mt-5 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4 text-emerald-950" role="status">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  >
                    <ExternalLink size={14} aria-hidden="true" /> Open verification page
                  </a>
                  <button
                    type="button"
                    ...
                    className={`inline-flex min-h-11 items-center gap-2 rounded-xl border border-emerald-300 bg-white px-3 py-2 text-xs font-semibold ${actionFocus}`}
                  >
                    <Copy size={14} aria-hidden="true" /> {copied ? 'Copied' : 'Copy verification link'}
                  </button>
                  <button
                    type="button"
          ...
          <article className="mt-6 rounded-2xl border bg-white p-6 text-slate-950 print:mt-0 print:rounded-none print:border-0 print:p-0" style={{ breakBefore: 'page' }}>
            <div className="flex flex-col gap-6 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0 max-w-xl">
                ...
                <h3 className="mt-2 break-words text-2xl font-extrabold">Invoice {document.invoiceNumber}</h3>
                ...
                <dl className="mt-5 grid gap-3 text-sm sm:grid-cols-2">
                  <div className="min-w-0"><dt className="font-semibold text-slate-500">Document ID</dt><dd className="mt-1 break-all font-mono text-xs">{document.documentId}</dd></div>
