  ...
  return (
    <div className="space-y-6">
      <section className="relative overflow-hidden rounded-[26px] border border-primary/14 bg-[hsl(var(--primary-soft))] px-5 py-7 shadow-[0_16px_40px_hsl(var(--foreground)/.035)] sm:px-7 sm:py-8">
        ...
        <p className="workspace-section-kicker">Public discovery</p>
        <h1 className="mt-2 max-w-3xl text-3xl font-bold tracking-[-.04em] sm:text-4xl">Prepare your verified public listing.</h1>
      ...
      <section className="rounded-2xl border bg-card p-5 shadow-[0_12px_30px_hsl(var(--foreground)/.03)] sm:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <p className="workspace-section-kicker">Verification status</p>
            <h2 className="mt-1 text-xl font-bold tracking-[-.025em]">Professional verification</h2>
            <div className="mt-4 flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold tracking-[-.015em]">{verificationLabels[status]}</h3>
      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        <div className="space-y-6">
          <section className="rounded-2xl border bg-card p-5 shadow-[0_12px_30px_hsl(var(--foreground)/.03)] sm:p-6">
            <div><p className="workspace-section-kicker">Patient-safe listing</p><h2 className="mt-1 text-xl font-bold tracking-[-.025em]">Public profile</h2></div>
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <label className="block space-y-2 sm:col-span-2"><span className="text-xs font-semibold text-foreground/70">Display name</span><input maxLength={120} value={draft.displayName} onChange={(event) => setField('displayName', event.target.value)} className={fieldClass} /><span className="text-[11px] text-muted-foreground">{draft.displayName.length}/120</span></label>
              <label className="block space-y-2 sm:col-span-2"><span className="text-xs font-semibold text-foreground/70">Headline</span><input maxLength={200} value={draft.headline} onChange={(event) => setField('headline', event.target.value)} className={fieldClass} /><span className="text-[11px] text-muted-foreground">{draft.headline.length}/200</span></label>
              <label className="block space-y-2 sm:col-span-2"><span className="text-xs font-semibold text-foreground/70">Short bio</span><textarea maxLength={2000} value={draft.bio} onChange={(event) => setField('bio', event.target.value)} className={textareaClass} /><span className="text-[11px] text-muted-foreground">{draft.bio.length}/2000</span></label>
              <label className="block space-y-2 sm:col-span-2"><span className="text-xs font-semibold text-foreground/70">Clinic / practice name <span className="font-normal">(optional)</span></span><input maxLength={160} value={draft.clinicName} onChange={(event) => setField('clinicName', event.target.value)} className={fieldClass} /><span className="text-[11px] text-muted-foreground">{draft.clinicName.length}/160</span></label>
            </div>
          </section>

          <section className="rounded-2xl border bg-card p-5 shadow-[0_12px_30px_hsl(var(--foreground)/.03)] sm:p-6">
            <p className="workspace-section-kicker">Care format</p>
            <h2 className="mt-1 text-xl font-bold tracking-[-.025em]">Service modes</h2>
            ...
            <div className="mt-5 space-y-3">
              {draft.serviceAreas.map((area, index) => (
                <div key={`${index}-${area.country_code}`} className="grid gap-3 rounded-2xl border bg-background/70 p-4 sm:grid-cols-[1fr_1fr_1fr_auto] sm:items-end">
                  <label className="space-y-2"><span className="text-xs font-semibold text-foreground/70">Locality</span><input maxLength={120} value={area.locality} onChange={(event) => updateArea(index, 'locality', event.target.value)} className="h-11 w-full rounded-xl border bg-card px-3 text-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" /></label>
                  <label className="space-y-2"><span className="text-xs font-semibold text-foreground/70">City</span><input maxLength={100} value={area.city} onChange={(event) => updateArea(index, 'city', event.target.value)} className="h-11 w-full rounded-xl border bg-card px-3 text-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" /></label>
                  <label className="space-y-2"><span className="text-xs font-semibold text-foreground/70">State</span><input maxLength={100} value={area.state} onChange={(event) => updateArea(index, 'state', event.target.value)} className="h-11 w-full rounded-xl border bg-card px-3 text-sm outline-none focus:border-primary focus:ring-4 focus:ring-primary/10" /></label>
          ...
          <section className="rounded-2xl border bg-card p-5 shadow-[0_12px_30px_hsl(var(--foreground)/.03)] sm:p-6">
            <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-primary/6 text-primary"><ShieldCheck size={20} /></span><div><p className="workspace-section-kicker">Visibility preference</p><h2 className="mt-1 text-lg font-bold tracking-[-.02em]">Public discovery opt-in</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">This preference means “publish this listing once I’m verified.” Public search still requires successful professional verification.</p></div></div>
            <label className={`mt-5 flex items-start gap-3 rounded-2xl border p-4 ${listingReady ? 'bg-background/70' : 'bg-muted/35'}`}><input type="checkbox" className="mt-1 size-4 accent-[hsl(var(--primary))]" disabled={!listingReady} checked={draft.isDiscoverable} onChange={(event) => setField('isDiscoverable', event.target.checked)} /><span><span className="block text-sm font-semibold text-foreground">Publish this listing once I’m verified</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">Requires a display name, at least one service mode and at least one complete service area.</span></span></label>
        ...
        <aside className="xl:sticky xl:top-24 xl:self-start">
          <section className="overflow-hidden rounded-[26px] border bg-card shadow-[0_16px_42px_hsl(var(--foreground)/.04)]">
            <div className="border-b bg-secondary/45 px-5 py-4"><div className="flex items-center gap-2"><Eye size={17} className="text-primary" /><h2 className="text-base font-bold tracking-[-.015em]">Patient preview</h2></div><p className="mt-1 text-xs text-muted-foreground">Only discovery-safe fields are shown here.</p></div>
            <div className="p-5 sm:p-6">
              <div className="flex items-start gap-4"><div className="grid size-14 shrink-0 place-items-center rounded-2xl border border-primary/12 bg-primary/7 text-base font-semibold text-primary">{draft.displayName.trim().split(/\s+/).filter(Boolean).slice(0, 2).map((part) => part[0]).join('').toUpperCase() || 'PT'}</div><div className="min-w-0"><h3 className="text-xl font-bold tracking-[-.02em]">{draft.displayName.trim() || 'Your display name'}</h3>{status === 'verified' && <div className="mt-1 inline-flex items-center gap-1.5 rounded-full bg-success/7 px-2.5 py-1 text-xs font-semibold text-success"><BadgeCheck size={14} /> Verified professional</div>}{draft.headline && <p className="mt-2 text-sm font-medium">{draft.headline}</p>}{draft.clinicName && <p className="mt-1 text-xs text-muted-foreground">{draft.clinicName}</p>}</div></div>
