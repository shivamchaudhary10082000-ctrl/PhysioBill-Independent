...
export function PatientInvoicePaymentInstructions({ invoiceId, outstanding }: Props) {
  ...
  if (!instructions) {
    return <div className="mt-4 rounded-xl border bg-secondary/20 p-4" aria-busy={loading}>
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          onClick={load}
          disabled={loading}
          className="inline-flex min-h-11 items-center justify-center gap-2 rounded-lg border bg-background px-4 py-2 text-sm font-semibold outline-none transition hover:bg-secondary focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-60"
        >
      ...
      <div className="mt-2 min-h-5 text-xs" role="status" aria-live="polite">
        {loading ? <span className="text-muted-foreground">Loading payment instructions securely…</span> : null}
  ...
  return <div className="mt-4 rounded-xl border border-primary/20 bg-primary/5 p-4">
    <div className="flex flex-wrap items-start justify-between gap-3">
      <div className="min-w-0 flex-1">
        ...
        <p className="mt-1 text-xs leading-5 text-muted-foreground">For <span className="break-all font-medium">{instructions.invoiceNumber}</span> · outstanding {money(instructions.outstanding)}. Sending money does not automatically prove settlement; payment status changes only through PhysioBill's controlled payment authority.</p>
      </div>
    ...
    {!destination ? <div className="mt-3 rounded-lg bg-background/80 p-3 text-sm text-muted-foreground" role="status">This physiotherapist has not configured an active default payment destination.</div> : null}

    {destination?.type === 'upi' ? <div className="mt-3 flex items-start gap-3 rounded-lg bg-background/80 p-3">
      <Smartphone size={18} className="mt-0.5 shrink-0 text-primary" aria-hidden="true" />
      <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[.1em] text-muted-foreground">{destination.label || 'UPI'}</p><p className="mt-1 break-all font-mono text-sm font-semibold">{destination.upiId}</p></div>
    </div> : null}
    ...
    {destination?.type === 'bank' ? <div className="mt-3 flex items-start gap-3 rounded-lg bg-background/80 p-3">
      ...
      <div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[.1em] text-muted-foreground">{destination.label || 'Bank transfer'}</p><p className="mt-1 break-words text-sm font-semibold">{destination.bankName || 'Bank'}</p><p className="mt-1 break-all font-mono text-sm">A/C {destination.accountNumberDisplay || '—'}</p><p className="break-all font-mono text-sm">IFSC {destination.ifscDisplay || '—'}</p></div>
    </div> : null}
