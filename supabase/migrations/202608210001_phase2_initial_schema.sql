...
create table if not exists public.physiotherapist_profiles (
  ...
  payment_account_status text not null default 'not_connected'
    check (payment_account_status in ('not_connected', 'pending', 'connected')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table if not exists public.patients (
  ...
  referral_date date,
  insurance_tpa text not null default '',
  policy_member_id text not null default '',
  notes text not null default '',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (physio_id, patient_number),
  unique (id, physio_id)
);
...
create table if not exists public.visits (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (physio_id, visit_number),
  unique (id, physio_id),
  foreign key (patient_id, physio_id)
    references public.patients(id, physio_id) on delete restrict
);
...
create table if not exists public.clinical_records (
  ...
  treatment text not null default '',
  hep text not null default '',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (visit_id),
  foreign key (patient_id, physio_id)
    references public.patients(id, physio_id) on delete restrict,
  foreign key (visit_id, physio_id)
    references public.visits(id, physio_id) on delete cascade
);
...
create table if not exists public.invoices (
  ...
  additional numeric(12,2) not null default 0 check (additional >= 0),
  ...
  payment_method text not null default 'Select payment method',
  finalized boolean not null default false,
  status text not null default 'Draft'
    check (status in ('Draft', 'Outstanding', 'Part paid', 'Paid')),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (physio_id, invoice_number),
  unique (id, physio_id),
  foreign key (patient_id, physio_id)
    references public.patients(id, physio_id) on delete restrict
);
...
create table if not exists public.invoice_audit_entries (
  ...
  action text not null check (action in ('correction', 'edit', 'payment')),
  ...
  foreign key (invoice_id, physio_id)
    references public.invoices(id, physio_id) on delete cascade
);
...
create table if not exists public.payments (
  ...
  status text not null default 'recorded'
    check (status in ('recorded', 'pending', 'succeeded', 'failed', 'refunded')),
  provider text,
  ...
  created_at timestamptz not null default now(),
  foreign key (invoice_id, physio_id)
    references public.invoices(id, physio_id) on delete restrict,
  foreign key (patient_id, physio_id)
    references public.patients(id, physio_id) on delete restrict
);
...
create or replace function public.owns_physio(target_physio_id uuid)
returns boolean
language sql
stable
security definer
...
create policy physiotherapists_select_self
on public.physiotherapists for select to authenticated
using (user_id = auth.uid());

create policy profiles_owner_all
on public.physiotherapist_profiles for all to authenticated
using (public.owns_physio(physio_id))
with check (public.owns_physio(physio_id));

create policy settings_owner_all
on public.physiotherapist_settings for all to authenticated
using (public.owns_physio(physio_id))
with check (public.owns_physio(physio_id));
...
on public.physio_patient_relationships for all to authenticated
using (public.owns_physio(physio_id))
with check (public.owns_physio(physio_id));
...
create policy clinical_records_owner_all
...
create policy invoice_audit_owner_select
on public.invoice_audit_entries for select to authenticated
using (public.owns_physio(physio_id));

create policy invoice_audit_owner_insert
on public.invoice_audit_entries for insert to authenticated
with check (
  public.owns_physio(physio_id)
...
-- Payment writes are intentionally not exposed directly to the browser yet.
-- A later authenticated server/database operation will record payments and
-- update invoice paid/status atomically.
