...
Deno.serve(async (req) => {
  ...
  if (!supabaseUrl || !publicKey || !serviceRoleKey) return json(500, { error: 'PDF service is not configured.' });
  ...
  if (!/^[0-9a-f-]{36}$/i.test(invoiceId)) return json(400, { error: 'Invalid invoice ID.' });
  ...
  const { data: snapshotRow, error: snapshotError } = await userClient
    ...
    .maybeSingle();
  if (snapshotError) return json(500, { error: 'Unable to load issued invoice.' });
  if (!snapshotRow) return json(404, { error: 'Issued invoice is unavailable.' });
  ...
  if (claimError || !Array.isArray(claimRows) || !claimRows[0]) return json(500, { error: 'Unable to claim PDF generation.' });
  ...
  const signedResponse = async (sha256: string, byteSize: number) => {
    ...
    if (error || !data?.signedUrl) return json(500, { error: 'Unable to create secure PDF download.' });
    return json(200, {
  ...
  try {
    if (action === 'complete') {
      ...
      if (!stored) return json(409, { error: 'Stored PDF artifact is missing.' });
      return await signedResponse(stored.hash, stored.bytes.byteLength);
    }
    if (action === 'in_progress') return json(409, { error: 'PDF generation is already in progress. Please retry shortly.' });
    if (action !== 'claimed') return json(500, { error: 'Unexpected PDF generation state.' });
    ...
    if (code.includes('mismatch')) return json(409, { error: 'Stored PDF integrity check failed.' });
    if (code === 'unsupported_pdf_text') return json(422, { error: 'This invoice contains text the current PDF renderer cannot safely encode.' });
    return json(500, { error: 'PDF generation failed.' });
  }
});
