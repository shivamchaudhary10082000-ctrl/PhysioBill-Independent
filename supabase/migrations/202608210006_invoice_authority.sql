...
begin
  ...
  if tg_op = 'INSERT' then
    ...
    invoice_year := extract(year from coalesce(new.start_date, current_date))::integer;
...
create trigger invoices_assign_identity
before insert or update on public.invoices
for each row
execute function private.assign_invoice_identity();
