...
-- Phase 5 service-location integration: expose only the authoritative active
-- therapist service-area UUID alongside the already-public coarse area fields.
-- The identifier is scheduling metadata only; it grants no identity, clinical,
-- attendance, invoice, payment, or settlement authority.
...
comment on function public.search_verified_therapists(text, text, text) is
  'Verified therapist discovery with active coarse service-area UUIDs for scheduling binding. Service-area identifiers confer no identity, clinical, attendance, or financial authority.';
